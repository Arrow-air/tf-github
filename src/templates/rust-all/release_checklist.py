...
def check_todos(filenames, include_tests):
    print("\U0001F6A7 Checking TODOs and FIXMEs...\n")
    statements = get_todos(filenames)
    ...
    print(
        fmt_string.format(
            "-"*5, "-"*length, "-"*50, length=longest_key
        )
    )
    for key in statements.keys():
        if statements[key]["tests"] and not include_tests:
...
if __name__ == "__main__":
    ...
    check_todos(filenames, args.tests)
    separator()
